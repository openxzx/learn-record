...
typedef struct _spi{
    ...
    int lgw_regpage;
    int spi_index;
    int (* lgw_spi_open)( void ** spi_target_ptr, int index);
...
typedef struct _lbt_conf{
    bool lbt_enable;
    uint8_t lbt_nb_active_channel;
    int8_t lbt_rssi_target_dBm;
    int8_t lbt_rssi_offset_dB;
    uint32_t lbt_start_freq;
    struct lgw_conf_lbt_chan_s lbt_channel_cfg[LBT_CHANNEL_FREQ_NB];
} t_lbt_conf;

typedef struct _rf_conf{
    bool rf_enable[LGW_RF_CHAIN_NB];
    uint32_t rf_rx_freq[LGW_RF_CHAIN_NB]; /* absolute, in Hz */
    float rf_rssi_offset[LGW_RF_CHAIN_NB];
    bool rf_tx_enable[LGW_RF_CHAIN_NB];
    uint32_t rf_tx_notch_freq[LGW_RF_CHAIN_NB];
    enum lgw_radio_type_e rf_radio_type[LGW_RF_CHAIN_NB];
} t_rf_conf;

typedef struct _if_conf{
    bool if_enable[LGW_IF_CHAIN_NB];
    bool if_rf_chain[LGW_IF_CHAIN_NB]; /* for each IF, 0 -> radio A, 1 -> radio B */
    int32_t if_freq[LGW_IF_CHAIN_NB]; /* relative to radio frequency, +/- in Hz */
} t_if_conf;



typedef struct _lgw_context{
    bool        lgw_is_started;
    t_rf_conf   rf_conf;
    t_if_conf   if_conf;
    uint32_t    reset_pin;
    ...
    int8_t cal_offset_a_i[8]; /* TX I offset for radio A */
    int8_t cal_offset_a_q[8]; /* TX Q offset for radio A */
    int8_t cal_offset_b_i[8]; /* TX I offset for radio B */
    int8_t cal_offset_b_q[8]; /* TX Q offset for radio B */
    uint8_t lora_multi_sfmask[LGW_MULTI_NB]; /* enables SF for LoRa 'multi' modems */
    ...
    t_lbt_conf lbt_conf;
    bool tx_notch_support;
    uint8_t tx_notch_offset;
...
lgw_context * lgw_context_init( LGW_SPI_TYPE type, int index );

void lgw_spi_init(LGW_SPI_TYPE type, t_spi * spi);
...
*/
int lgw_board_setconf(struct lgw_conf_board_s conf, lgw_context * ctx);
...
@return LGW_HAL_ERROR id the operation failed, LGW_HAL_SUCCESS else
*/
int lgw_lbt_setconf(struct lgw_conf_lbt_s conf, lgw_context * ctx);
...
@return LGW_HAL_ERROR id the operation failed, LGW_HAL_SUCCESS else
*/
int lgw_rxrf_setconf(uint8_t rf_chain, struct lgw_conf_rxrf_s conf, lgw_context * ctx);
...
@return LGW_HAL_ERROR id the operation failed, LGW_HAL_SUCCESS else
*/
int lgw_rxif_setconf(uint8_t if_chain, struct lgw_conf_rxif_s conf, lgw_context * ctx);
...
*/
int lgw_receive(uint8_t max_pkt, struct lgw_pkt_rx_s *pkt_data, lgw_context * ctx);
...
the protocol.
*/
int lgw_send(struct lgw_pkt_tx_s pkt_data,lgw_context * ctx);
...
@return LGW_HAL_ERROR id the operation failed, LGW_HAL_SUCCESS else
*/
int lgw_status(uint8_t select, uint8_t *code, lgw_context * ctx);

/**
@brief Abort a currently scheduled or ongoing TX
@return LGW_HAL_ERROR id the operation failed, LGW_HAL_SUCCESS else
*/
int lgw_abort_tx(t_spi * spi);
...
@return LGW_HAL_ERROR id the operation failed, LGW_HAL_SUCCESS else
*/
int lgw_get_trigcnt(uint32_t* trig_cnt_us, t_spi * spi);
...
*/
uint32_t lgw_time_on_air(struct lgw_pkt_tx_s *packet, uint8_t fsk_sync_word_size);

uint32_t lgw_rx_time_on_air(struct lgw_pkt_rx_s *packet, uint8_t fsk_sync_word_size);
